...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((res, rej) => {
    if (typeof isPositiveAnswer !== 'boolean') return rej(new Error('Wrong parameter is passed! Ask her again.'));
    if (!isPositiveAnswer) return res('Oh no, she said "No".');
...
function chainPromises(array, action) {
  return new Promise((resolve) => {
    const result = [];
    array.map((promise) => promise.then((x) => result.push(x), (x) => new Error(x)));
    resolve(result);
